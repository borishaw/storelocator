 ...
 * @version     $Id: sef_ext.php 24 2007-09-19 18:35:29Z silianacom-svn $ 
...
class sef_404
{
    ...
    // V 1.2.4.j shAppendString will be added to sef url, but not saved in DB or cache 
	{
		...
		// then look for sh404SEF own plugin 
			...
			include(sh404SEF_ABS_PATH.'components/com_sef/sef_ext/'.$option.'.php');
			}
    		// else if no plugin nowhere default to Joomla own sef
    		else {
		 /*
		  $shDump = '';
		  $shDump .= 'string = '.$string."\n";
		  $shDump .= 'shLanguage = '.$shLanguage."\n";
		  $shDump .= 'Dumping vars :'."\n";
		  $shDump .= print_r($vars, true)."\n";
		  $shDump .= 'title = '. print_r($title, true)."\n";
		  $shDumpFile = fopen(sh404SEF_ABS_PATH.'sh404SEF_dump.txt', 'a');
		  if ($shDumpFile) {
        fwrite( $shDumpFile, $shDump);
        fclose( $shDumpFile);
      }
      */
	...
	function revert(&$url_array, $pos)
	{
		...
		if ($result) {
			...
			//parse_str($QUERY_STRING,$vars);  // V x unneeded, and cause issues when logging in
			//$_GET = array_merge($_GET,$vars); // V x 
			//$_REQUEST = array_merge($_REQUEST,$vars); // V x 
		}
		return $QUERY_STRING;
	}
