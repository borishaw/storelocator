...
switch ($URI->path) {
    ...
    case $base.$index: { 
        ...
        if (is_null($option)) {
    		...
    		if (empty($URI->querystring) 
        		...
        		$current = $URI->protocol.'://'.$URI->host.$shSaveRequestURI.$shAnchor;  
        		if ($dest != $current) 
        			shRedirect($dest);
    		} 
    ...
    default: {
        ...
        // TODO : should we also check security here ? coming from the DB, we should be safe
        ...
        if (is_valid($QUERY_STRING)) { 
        	...
        	if ((isset($_GET['option']) && ($_GET['option'] == 'login' || $_GET['option'] == 'logout')) ||
        		...
        		$REQUEST_URI = $GLOBALS['mosConfig_live_site'].'/index.php';
        		$_SERVER['REQUEST_URI'] = $REQUEST_URI;
            ...
            if (!empty($shLastBit)   // if URl does not end with a / 
              ...
              && ( empty($sefConfig->addfile) 
                ...
                if (is_valid($shQueryString)) {  //let's redirect to this new URL
                	...
                	$dest = ltrim( str_replace($mosConfig_live_site, '', $dest), '/');
                	shRedirect($mosConfig_live_site.$sefConfig->shRewriteStrings[$sefConfig->shRewriteMode].$dest.'/');
                }
              }
            ...
            $mosmsg = ' ('.$mosConfig_live_site.'/'.$path.')'; // V 1.2.4.t
            $_GET['mosmsg'] = $_REQUEST['mosmsg'] = $mosmsg;
            $option="com_content";
            $task="view";
            //$Front404 = 0;
            // V 1.2.4.t  set Itemid to homepage
...
function shAddPaginationInfo( $limit, $limitstart, $iteration, $url, $location, $shSeparator = null){
  ...
  if ( (strpos($url,'option=com_search')) || (strpos($url,'option=com_content') &&
       ...
       //|| (strpos( $url, 'task=blogsection'))
        ...
        || (strpos($url,'option=com_virtuemart') && sh404SEF_COMPAT_VIRTUEMART_SHOW_LIMIT_AND_LIMITSTART)) {
  ...
  // add page number 
  if (!$shPageNumberWasReplaced && isset($limitstart) && ($limitstart != 0)) {
...
function sefRelToAbs($string, $shLanguageParam = null) {
    ...
    )
    {
        ...
        if  ((shIsHomePage($string)) || ($string == 'index.php')  // 10/08/2007 18:13:43 
          ...
          if (!empty($shMosMsg)) // V x 01/09/2007 22:48:01
            $ret .= (empty($shAppendString) ? '?':'&').$shMosMsg;               
        ...
        if (isset($option) && !($option=='com_content' && @$task == 'edit')) { // V x 29/08/2007 23:19:48
            ...
            switch ($shComponentType) {
                ...
                case 'noCache': {
                    ...
                    $sefstring = 'component/';
                    $URI = new sh_Net_URL($shSaveString);
                    if (count($URI->querystring) > 0) {
                      foreach($URI->querystring as $key => $value) {
                        $sefstring .= "$key,$value/";
                      }
            ...
            $string =  $GLOBALS['mosConfig_live_site'].$shRewriteBit.ltrim( $sefstring, '/')
               .$shAppendString
               . (empty($shMosMsg) ? '' : (empty($shAppendString) ? '?':'&').$shMosMsg) // V x 01/09/2007 22:58:24
               .(($URI->anchor)?"#".$URI->anchor:'');
        } 
...
function getMenuTitle($option, $task, $id = null, $string = null, $shLanguage = null)
{
    ...
    elseif (!empty($option)) {
    	...
    	$title = '/'; // don't know what else we could do, just go home
    }
    $database->setQuery($sql);
