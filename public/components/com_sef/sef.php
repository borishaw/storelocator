 ...
 * @version     $Id: sef.php 24 2007-09-19 18:35:29Z silianacom-svn $
	...
	if ($sefConfig->Enabled) {
		...
		if (is_readable($sef404)) {
			...
			if (isset ($_SERVER['REQUEST_URI'])) {
				...
				//$URI->path = str_replace('%3F','?' , $URI->path);
			}else{
			...
			if ($sefConfig->shAutoRedirectWww) {	  // mode = false do nothing, true = auto-redirect 
				...
				//$shTemp = explode('/', str_replace('http://', '', $GLOBALS['mosConfig_live_site']) );
				$shTemp = explode('/', str_replace($URI->protocol.'://', '', $GLOBALS['mosConfig_live_site']) );
				$shLiveSite = strtolower($shTemp[0]);
				if ($shLiveSite != strtolower($URI->host)) {
			    	if     (substr( $shLiveSite, 0, 4) == 'www.' 
              			 && strtolower('www.'.$URI->host) == $shLiveSite) {
            			shRedirect( $URI->protocol.'://www.'.$URI->host.$shSaveRequestURI);
          			}
          			if     (substr( $shLiveSite, 0, 4) != 'www.' 
            	  	  && strtolower(substr( $URI->host, 4)) == $shLiveSite) {
            			shRedirect($URI->protocol.'://'.str_replace('www.', '', $URI->host).$shSaveRequestURI);
          			}
        		}
      		}
			...
			if (strpos($GLOBALS['mosConfig_live_site'],$URI->host) === false) {
        		...
        		$shCurrentPageURL =  rtrim( $URI->protocol.'://'.$URI->host.$URI->path, '/');
