...
if (!defined( '_MOS_SUBMENU_MODULE' )) {
	...
	*/
	...
	function rtGetSubMenuLink( $mitem, $level, $hilight=false , $color_index=false) {
		...
		$rowcolor = "bg" . $tab_index++%2;

		// Active Menu highlighting
		$current_itemid = trim( mosGetParam( $_REQUEST, 'Itemid', 0 ) );
	
		if (($forcehilite && $hilight) || (($current_itemid == $mitem->id || $hilight) && !$forcehilite)) {
			if ($level == 0) {
				$tab_color = $id;
				$menuname = $mitem->name;
				$hilightid = $mitem->id;
			} 
			$start = "<li class=\"" . $rowcolor . " active\"><span>";
		} else {
			$start = "<li class=\"" . $rowcolor . "\"><span>";
		}
		...
		switch ($mitem->browserNav) {
			...
			$txt .= '<a href="'. $mitem->link .'" target="_blank">'. $mitem->name ."</a>";
			break;
			...
			case 3:
			// don't link it
			$txt .= '<span class="seperator">'. $mitem->name ."</span>";
			break;
	...
	function rtShowHorizMenu(  $menutype) {
		global $database, $my, $cur_template, $Itemid, $hilightid, $forcehilite, $tab_index;
	...
	*/
	function rtShowSubMenu(  $menutype, $suffix=""  ) {
		...
		if (isset($children[$id]) && $children[$id]) {
			$sidenav = rtRecurseSubMenu( $id, 1, $children, $open, $suffix);
		}
	...
	* hierarchial menu
	*/
	function rtRecurseSubMenu( $id, $level, &$children, &$open, $suffix) {
		...
		if (@$children[$id]) {
			...
			foreach ($children[$id] as $row) {
				...
				if ( in_array( $row->id, $open )) {
					$output .= rtRecurseSubMenu( $row->id, $level+1, $children, $open, $suffix );
				}
				$output .= "</li>\n";
			}
