...
// after or before the first occurence of the tag
  ...
  if (!function_exists('shCleanUpTitle')) {
    function shCleanUpTitle( $title) {
      return trim(stripslashes(htmlspecialchars( $title )));
    }
  } 
  ...
  if (!function_exists('shDoTitleTags')) {
    ...
    if ($sefConfig->shMetaManagementActivated) {
      ...
      global $option;
      if (file_exists($GLOBALS['mosConfig_absolute_path'].'/components/com_sef/meta_ext/'.$option.'.php'))
	      include_once($GLOBALS['mosConfig_absolute_path'].'/components/com_sef/meta_ext/'.$option.'.php');
      ...
      if (!is_null($shCustomTitleTag))
        $buffer = eregi_replace( '<\s*title\s*>.*<\s*\/title\s*>', '<title>'.shCleanUpTitle($shCustomTitleTag).'</title>', $buffer);
      ...
      if (!is_null($shCustomKeywordsTag))
        $buffer = preg_replace( '/\<\s*meta\s+name\s*=\s*"keywords.*\/\>/isU', '<meta name="keywords" content="'
                  .shCleanUpTitle($shCustomKeywordsTag).'" />', $buffer);  
  ...
  */
    ...
    $shPage = shDoTitleTags( $shPage);
     ...
     // to the page   
     initGzip();    // improved compat with other extensions
